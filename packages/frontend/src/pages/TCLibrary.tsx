...
export default function TCLibrary() {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        {/* Stats row */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '12px', flexShrink: 0 }}>
          ...
          <StatTile
            label="Coverage %"
            value={(() => {
              const total = stats?.totalTCs ?? allTCs.length;
              const auto = stats?.automatedCount ?? allTCs.filter(tc => !!tc.linkedScriptId).length;
