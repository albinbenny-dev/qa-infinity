...
function parseRobotXmlReport(xmlPath) {
  ...
  const tests = [];
  const testRegex = /<test\s+id="[^"]*"\s+name="([^"]*)"[^>]*>[\s\S]*?<status\s+status="(PASS|FAIL)"[^>]*(?:start="([^"]*)"[^>]*end="([^"]*)")?[^/]*/g;
  let m;
  while ((m = testRegex.exec(xml)) !== null) {
    const name = m[1];
    const status = m[2];
    const startStr = m[3];
    const endStr = m[4];
    ...
    if (startStr && endStr) {
      try {
        durationMs = new Date(endStr).getTime() - new Date(startStr).getTime();
      } catch { /* ignore */ }
    }
    // Extract error message for failed tests
    let errorMsg = null;
    const errorMatch = new RegExp(`<test[^>]*name="${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>[\\s\\S]*?<msg level="FAIL">([\\s\\S]*?)<\\/msg>`).exec(xml);
    if (errorMatch) errorMsg = errorMatch[1].replace(/<[^>]+>/g, '').trim();
...
const CONFIG_CONTENT = `module.exports = {
  ...
  use: {
    ...
    video: 'retain-on-failure', // only record video on failure — reduces Xvfb CPU overhead during passing runs
  },
  projects: [
